...
// src/components/dados/CampanhaCard.tsx — v4.3
// v4.3: TipoCampanhaBadge integrado — exibe tipo de campanha + métrica principal em cada card
...
import { TipoCampanhaBadge } from "@/components/TipoCampanhaBadge";
import type {
  CampanhaEnriquecida, CTA, Alerta, ScoreBadge, Periodo,
} from "@/app/analytics/types";
import type { Flag, ScoresAvancados, MediaConta, ScoresCampanha } from "@/app/lib/algoritmoErizon";
import { formatBRL, fmtBRL0 } from "@/app/analytics/engine";

// Tipo union — aceita ambos
type ScoresCompat = ScoresAvancados | ScoresCampanha;

// ─── helpers para compatibilidade ────────────────────────────────────────────
function getNarrativaTipo(scores: ScoresCompat): "sucesso" | "atencao" | "risco" | "neutro" {
  if ("narrativaTipo" in scores && scores.narrativaTipo) return scores.narrativaTipo;
  if (scores.urgencia === "critico")      return "risco";
  if (scores.urgencia === "oportunidade") return "sucesso";
  if (scores.urgencia === "atencao")      return "atencao";
  return "neutro";
}

function getNarrativa(scores: ScoresCompat): string {
  if ("narrativa" in scores && scores.narrativa) return scores.narrativa;
  if (scores.urgencia === "critico")
    return `Campanha com score ${scores.score}/100 operando em risco. ROAS ${scores.roas.toFixed(2)}× abaixo do mínimo.`;
  if (scores.urgencia === "oportunidade")
    return `Campanha com ROAS ${scores.roas.toFixed(2)}× e margem saudável. Pronta para escala.`;
  if (scores.urgencia === "atencao")
    return `Campanha exige monitoramento. CPL R$${scores.cpl.toFixed(0)} — revisar performance.`;
  return `Campanha operando dentro dos parâmetros esperados. Score ${scores.score}/100.`;
}

function getUrgenciaLabel(scores: ScoresCompat): string {
  if ("urgenciaLabel" in scores && scores.urgenciaLabel) return scores.urgenciaLabel;
  const labels: Record<string, string> = {
    critico: "Crítica", atencao: "Atenção", estavel: "Estável", oportunidade: "Escalar",
  };
  return labels[scores.urgencia] ?? "—";
}
...
// ─── FlagBadge ────────────────────────────────────────────────────────────────
const FLAG_S: Record<string, string> = {
...
export function FlagBadge({ flag }: { flag: Flag }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] font-semibold ${style}`}
...
// ─── NarrativaCard ────────────────────────────────────────────────────────────
const NAR_S: Record<string, { border: string; bg: string; text: string }> = {
...
export function NarrativaCard({ scores }: { scores: ScoresCompat }) {
  const tipo = getNarrativaTipo(scores);
  const s    = NAR_S[tipo] ?? NAR_S.neutro;
  const txt  = getNarrativa(scores);
  return (
    <div className={`px-4 py-3 rounded-xl border ${s.border} ${s.bg} mb-4`}>
      <p className={`text-[12px] leading-relaxed ${s.text}`}>{txt}</p>
    </div>
  );
}
...
export function AlertaChip({ alerta }: { alerta: Alerta }) {
  ...
  }[alerta.tipo] ?? "bg-white/[0.04] text-white/30 border-white/[0.06]";
  return (
...
export function CTAButton({ cta, campanha, scoreAntes, onDecisao }: {
      ...
      <>
        ...
        {modalAberto && (
          <ModalSimulacaoEscala
            campanha={{ ...campanha, nome_campanha: campanha.nome_campanha, gasto_total: campanha.gasto_total, contatos: campanha.contatos, score: scoreAntes }}
            onConfirmar={async () => {
              await onDecisao(campanha.id, cta.label, `Escala de 20% confirmada após simulação — Score: ${scoreAntes}`, { lucro: campanha.m.lucro, margem: campanha.m.margem });
            }}
  ...
  async function handleClick() {
    ...
    setEstado("loading");
    const impacto = cta.acao === "pausar" ? `Pausa solicitada — Score: ${scoreAntes}` : `Revisão: ${cta.label}`;
  ...
  return (
    <button onClick={handleClick} disabled={estado === "loading"}
...
function PainelAcoes({
  campanha, score, onDecisao, delay,
}: {
  campanha: CampanhaEnriquecida;
  score: number;
  onDecisao: (id: string, acao: string, impacto: string, extra?: { lucro?: number; margem?: number }) => Promise<void>;
  ...
  const [estadoPausar,  setEstadoPausar]  = useState<"idle" | "loading" | "done">("idle");
  const [estadoEscalar, setEstadoEscalar] = useState<"idle" | "loading" | "done">("idle");
  const [estadoRevisar, setEstadoRevisar] = useState<"idle" | "loading" | "done">("idle");
  const [modalEscala,   setModalEscala]   = useState(false);
  const [visivel, setVisivel] = useState(false);

  useEffect(() => {
    const t = setTimeout(() => setVisivel(true), delay + 100);
    return () => clearTimeout(t);
  }, [delay]);

  async function executar(
    tipo: "pausar" | "escalar" | "revisar",
    label: string,
    setEstado: (s: "idle" | "loading" | "done") => void
  ) {
    setEstado("loading");
    const impacto = tipo === "pausar"
      ? `Campanha pausada — Score: ${score}/100 · Lucro est.: R$${fmtBRL0(campanha.m.lucro)}`
      : tipo === "escalar"
      ? `Escala 20% registrada — Score: ${score}/100 · ROAS: ${campanha.m.roas.toFixed(2)}×`
      : `Revisão registrada — Score: ${score}/100`;
    await onDecisao(campanha.id, label, impacto, { lucro: campanha.m.lucro, margem: campanha.m.margem });
    setEstado("done");
    setTimeout(() => setEstado("idle"), 3000);
  }

  const isPausar  = score < 60;
  const isEscalar = score >= 75;

  return (
    <div
      className="mt-4 pt-4 border-t border-white/[0.04]"
      style={{ opacity: visivel ? 1 : 0, transition: `opacity 0.4s ease ${delay + 150}ms` }}
    >
      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-white/15 mb-3">
        Registrar decisão
      </p>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {/* Pausar */}
        {estadoPausar === "done" ? (
          <span className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-medium text-emerald-400">
            <CheckCircle2 size={12} /> Pausa registrada
          </span>
        ) : (
          <button
            onClick={() => executar("pausar", "Pausar campanha", setEstadoPausar)}
            disabled={estadoPausar === "loading"}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-[12px] font-medium transition-all disabled:opacity-50 ${
              isPausar
                ? "bg-red-500/10 border-red-500/25 text-red-400 hover:bg-red-500/15"
                : "bg-white/[0.03] border-white/[0.08] text-white/30 hover:text-red-400 hover:border-red-500/20"
            }`}
          >
            {estadoPausar === "loading" ? <Loader2 size={12} className="animate-spin" /> : <PauseCircle size={12} />}
            {estadoPausar === "loading" ? "Pausando..." : "Pausar"}
          </button>
        )}
        ...
        {estadoEscalar === "done" ? (
          <span className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-medium text-emerald-400">
            <CheckCircle2 size={12} /> Escala registrada
          </span>
        ) : (
          <>
            <button
              onClick={() => setModalEscala(true)}
              disabled={estadoEscalar === "loading"}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-[12px] font-medium transition-all disabled:opacity-50 ${
                isEscalar
                  ? "bg-emerald-500/10 border-emerald-500/25 text-emerald-400 hover:bg-emerald-500/15"
                  : "bg-white/[0.03] border-white/[0.08] text-white/30 hover:text-emerald-400 hover:border-emerald-500/20"
              }`}
            >
              <TrendingUp size={12} /> Escalar 20%
            </button>
            {modalEscala && (
              <ModalSimulacaoEscala
                campanha={{ ...campanha, nome_campanha: campanha.nome_campanha, gasto_total: campanha.gasto_total, contatos: campanha.contatos, score }}
                onConfirmar={async () => {
                  setModalEscala(false);
                  await executar("escalar", "Escalar 20%", setEstadoEscalar);
                }}
                onFechar={() => setModalEscala(false)}
              />
            )}
          </>
        )}
        ...
        {estadoRevisar === "done" ? (
          <span className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-[12px] font-medium text-emerald-400">
            <CheckCircle2 size={12} /> Revisão registrada
          </span>
        ) : (
          <button
            onClick={() => executar("revisar", "Revisar segmentação e criativos", setEstadoRevisar)}
            disabled={estadoRevisar === "loading"}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-white/[0.08] bg-white/[0.03] text-white/30 hover:text-amber-400 hover:border-amber-500/20 text-[12px] font-medium transition-all disabled:opacity-50"
          >
            {estadoRevisar === "loading" ? <Loader2 size={12} className="animate-spin" /> : <RotateCcw size={12} />}
            {estadoRevisar === "loading" ? "Registrando..." : "Revisar"}
          </button>
        )}
      </div>
    </div>
  ...
  return (
    <div className="p-6 rounded-2xl bg-[#0f0f11] border border-white/[0.05] mb-5">
      ...
      <div className="h-[3px] rounded-full bg-white/[0.05] overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-1000 ease-out ${color}`} style={{ width: animado ? `${score}%` : "0%" }} />
      </div>
    </div>
  );
}
...
export function EmptyState({ periodo, filtrando, onLimpar }: { periodo: Periodo; filtrando: boolean; onLimpar: () => void }) {
  const labels: Record<Periodo, string> = { hoje: "hoje", "7d": "nos últimos 7 dias", "30d": "nos últimos 30 dias", mes: "este mês" };
  return (
    ...
    >
      <div className="flex items-start gap-5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between gap-3 mb-2">
            <div>
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                ...
                {scores && (
                  <UrgenciaBadge urgencia={scores.urgencia} label={getUrgenciaLabel(scores)} />
                )}
              </div>

              {/* ── v4.3: Badge de tipo de campanha ── */}
          ...
          {/* Painel de Decisão */}
          <PainelAcoes campanha={c} score={m.score} onDecisao={onDecisao} delay={delay} />
