...
/**
 * InteligenciaResumo.tsx — v4.0
 * Reescrito para usar ContaHealth (HealthResult) do algoritmoErizon v6.2
 */
...
import type { HealthResult, RecomendacaoAcao } from "@/app/lib/algoritmoErizon";
...
export default function InteligenciaResumo({ health }: { health: HealthResult }) {
  ...
  const lucroTotal = health.enriched.reduce((s, c) => s + c.lucro, 0);
  const margemPct  = (health.margemMedia * 100).toFixed(1);

  // Índices derivados (não existem como coluna direta, estimados)
  const ceMedia   = clamp(health.mediaConta.roas * 20 + (100 - health.mediaConta.cpl / 2));
  const satMedia  = clamp(health.mediaConta.freq > 0 ? (health.mediaConta.freq / 5) * 100 : 30);
  const alavMedia = clamp(health.roasMedio * 20);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {health.lucroPerda7d > 10 && (
                  <ProjecaoCard tipo="risco" receita={health.lucroPerda7d} lucro={health.lucroPerda7d * 0.3}
                    descricao="Perda projetada se campanhas críticas continuarem" />
                )}
                {health.ganhoProjetado7d > 10 && (
                  <ProjecaoCard tipo="ganho" receita={health.ganhoProjetado7d} lucro={health.ganhoProjetado7d * 0.7}
                    descricao="Receita adicional escalando campanhas prontas" />
                )}
              </div>
            </div>
          )}
          ...
          {health.recomendacoes.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {health.recomendacoes.map((rec, i) => <RecCard key={i} rec={rec} />)}
              </div>
            </div>
          )}
