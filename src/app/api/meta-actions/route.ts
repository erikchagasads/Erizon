...
const MetaActionBodySchema = z.object({
  ...
  action: z.enum(["PAUSE", "RESUME", "UPDATE_BUDGET"] as const),
...
export async function POST(req: Request) {
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(validationError(parsed), { status: 422 });
    }

    const { campaignId, action, value } = parsed.data;
    ...
    if (action === "PAUSE") {
      actionBody = { status: "PAUSED" };
    } else if (action === "RESUME") {
      actionBody = { status: "ACTIVE" };
    } else if (action === "UPDATE_BUDGET") {
      if (!value || isNaN(Number(value))) {
        return NextResponse.json({ error: "Valor de budget inválido." }, { status: 400 });
      }
      actionBody = { daily_budget: Math.round(Number(value) * 100) }; // Meta usa centavos
    } else {
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({ ...actionBody, access_token: ACCESS_TOKEN }),
    });
