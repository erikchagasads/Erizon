...
export async function POST(req: Request) {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return NextResponse.json({ text: "Configuração pendente: Adicione a GEMINI_API_KEY na Vercel." }, { status: 500 });
    }

    // Usando gemini-1.5-flash que é o mais rápido e estável para chaves gratuitas
