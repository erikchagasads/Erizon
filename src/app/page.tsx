...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen bg-[#0f1013] text-[#e4e4e7] font-sans selection:bg-[#6c4bff]/30">
      ...
      <aside className="w-64 bg-[#16171a] flex flex-col border-r border-white/5 shadow-2xl z-10">
        ...
        <nav className="flex-1 px-4 space-y-1">
          ...
          <button className="w-full flex items-center gap-3 p-4 bg-[#6c4bff] rounded-[22px] text-white text-sm font-bold shadow-lg shadow-[#6c4bff]/20 transition-all active:scale-95">
            📊 Overview
          </button>
        ...
        <div className="p-6">
          <button 
            ...
            className="w-full p-4 bg-[#1c1d21] border border-white/5 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 hover:text-red-500 transition-all duration-300"
          >
            Encerrar_Sessão
          </button>
        </div>
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <header className="flex justify-between items-center mb-10 bg-[#16171a] p-10 rounded-[45px] border border-white/5 shadow-2xl relative overflow-hidden">
          <div className="z-10">
            ...
            <p className="text-[#6c4bff] text-[10px] font-black mt-4 opacity-90 uppercase tracking-[0.3em] flex items-center gap-2">
               <span className="w-1.5 h-1.5 bg-[#6c4bff] rounded-full animate-ping"></span> Operador: {user?.email}
            </p>
          </div>
        ...
        <div className="mb-10 text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {metrics.map((item) => (
              <div key={item.id} className="bg-[#1c1d21] border border-white/5 p-10 rounded-[50px] hover:border-[#6c4bff]/40 transition-all duration-500 group relative shadow-2xl hover:-translate-y-1">
                ...
                <div className="flex justify-between items-end">
                  ...
                  <div className={`${item.is_positive ? 'text-[#00ff9d]' : 'text-red-500'}`}>
                    <span className="text-[10px] font-black bg-white/5 px-4 py-2 rounded-2xl border border-white/5 uppercase shadow-sm">
                      {item.change}
                    </span>
                  </div>
                </div>
        ...
        {/* GRÁFICO */}
        <div className="bg-[#16171a] border border-white/5 p-12 rounded-[60px] shadow-2xl overflow-hidden relative group">
          ...
          <div className="h-96 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={history} margin={{ left: 10, right: 30, top: 0, bottom: 0 }}>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#1c1d21', border: 'none', borderRadius: '24px', padding: '15px', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }} itemStyle={{ color: '#6c4bff', fontWeight: 'bold' }} />
                <Area type="monotone" dataKey="amount" stroke="#6c4bff" strokeWidth={6} fill="url(#purpleGlow)" dot={{ fill: '#6c4bff', r: 6, strokeWidth: 4, stroke: '#16171a' }} activeDot={{ r: 10, fill: '#00ff9d', strokeWidth: 0 }} animationDuration={2500} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
